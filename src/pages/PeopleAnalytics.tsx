// src/pages/PeopleAnalytics.tsx
import { useEffect, useRef, useState } from "react";
import Hls from "hls.js";
import { API_BASE, ANALYTICS_BASE, type Stream, type AnalyticsResponse } from "../config";
      ...
      <div className="stream-header">
        ...
        <div className="stream-meta">
          ...
          <div className="stream-subtitle">
            {stream.place || "Unknown Location"} — People Analytics
          </div>
        </div>
      </div>
      ...
      {data && (
        <div style={{ marginTop: "0.8rem", display: "grid", gap: "0.5rem" }}>
          <div style={{ display: "flex", gap: "1.2rem", flexWrap: "wrap" }}>
            <div>
              <div className="card-subtitle">Unique People (window)</div>
              <div style={{ fontSize: "1.4rem", fontWeight: 600 }}>
                {data.totalUnique}
              </div>
            </div>
            <div>
              <div className="card-subtitle">People in Latest Frame</div>
              <div style={{ fontSize: "1.4rem", fontWeight: 600 }}>
      ...
      try {
        ...
        const results = await Promise.all(
          subset.map(async (stream) => {
            try {
              const url = `${ANALYTICS_BASE}/analytics/people?streamKey=${encodeURIComponent(
                stream.streamKey
              )}`;
    ...
    <div className="page">
      <div className="page-header">
        ...
        <p className="card-subtitle">
          Live YOLO-based crowd estimates for all currently active temple drone streams.
        </p>
      </div>
